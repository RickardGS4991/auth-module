...
public class TokenMapper {
    public RefreshTokensEntity toPersistance(RefreshTokens token){
        return new RefreshTokensEntity(token.showRefreshTokenId(), token.getUserId(), token.showRToken(), token.getExpiresAt(), token.isRevoked());
    }

    public RefreshTokens toDomain(RefreshTokensEntity token){
        return RefreshTokens.restore(token.getRefreshTokenId(), token.getUserId(), token.getRefreshToken(), token.getExpiresAt(), token.getRevokedState());
    }
}
